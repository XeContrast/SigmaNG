...
public class GradientRoundRectShader {
    ...
    public static void applyGradient(float x, float y, float width, float height, float alpha, Color bottomLeft,Color color2) {
        ...
        gradientMaskShader.setUniformf("location", (float) (x * sr.getGuiScaleFactor()), (float) ((Minecraft.getInstance().getMainWindow().getHeight() - (height * sr.getGuiScaleFactor())) - (y * sr.getGuiScaleFactor())));
        gradientMaskShader.setUniformf("rectSize", (float) (width * sr.getGuiScaleFactor()), (float) (height * sr.getGuiScaleFactor()));
