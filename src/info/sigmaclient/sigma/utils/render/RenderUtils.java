...
//        GlStateManager.enableLighting();
    ...
    public static void drawTextureLocationZoom(final double x, final double y, final double width, final double height, final String image, Color c,boolean b) {
        GL11.glPushMatrix();
        startBlend();
        GlStateManager.enableTexture2D();
        Minecraft.getInstance().getTextureManager().bindTexture(new ResourceLocation( b? (image.contains(".") ? image : (image + ".png")) : ("sigmang/images/" + (image.contains(".") ? image : (image + ".png"))) ));
        GlStateManager.resetColor();
        GlStateManager.color(c.getRed() / 255.0f, c.getGreen() / 255.0f, c.getBlue() / 255.0f, c.getAlpha() / 255.0f);
...
//        com.mojang.blaze3d.platform.GlStateManager.enableAlphaNoLock();
    ...
    public static void drawGoodCircle(double x, double y, float radius, int color) {
        ...
        GLUtil.setup2DRendering(() -> {
            ...
            GLUtil.render(GL_POINTS, () -> glVertex2d(x + radius/2, y + radius/2));
        });
