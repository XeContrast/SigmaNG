...
public class Sigma5CheckBox extends Button
{
    ...
    public void renderButton(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        float ex = 0;
        䬹鞞葫Ꮀ待湗2((float)this.x - ex, (float)this.y - ex, (float)this.width + ex * 2, (float)this.height + ex * 2, 11f, 霥瀳놣㠠釒(-4144960, (this.isHovered() ? 0.6f : 0.43f) * this.卫鷏䈔Ⱋ㦖.getAnim() * 1));
        final float n2 = (1.0f - this.卫鷏䈔Ⱋ㦖.getAnim()) * 1;
        䬹鞞葫Ꮀ待湗2((float)this.x - ex, (float)this.y - ex, (float)this.width + ex * 2, (float)this.height + ex * 2, 11f, 霥瀳놣㠠釒(贞䩉㥇딨햖(-14047489, -16711423, this.isHovered() ? 0.9f : 1.0f), n2));
        GL11.glPushMatrix();
