...
public final class TargetStrafe extends Module {
    ...
    public static ModeValue modeValue = new ModeValue("Type", "Smart", new String[]{"Smart", "Control","Legit"});
    ...
    public static void onStrafe(StrafeEvent event){
        ...
        if(!canStrafe())return;
        if (TargetStrafe.onlySpace.isEnable() && !mc.gameSettings.keyBindJump.pressed) {
            return;
        }
        ...
        if(modeValue.is("Legit")){
            float yaw = RotationUtils.calculate(Killaura.attackTarget).x + (90 + 45) * direction;

            final double range = radius.getValue().doubleValue() + Math.random() / 100f;
            final double posX = -MathHelper.sin((float) Math.toRadians(yaw)) * range + Killaura.attackTarget.getPosX();
            final double posZ = MathHelper.cos((float) Math.toRadians(yaw)) * range + Killaura.attackTarget.getPosZ();

            yaw = RotationUtils.calculate(new Vector3d(posX, Killaura.attackTarget.getPosY(), posZ)).x;
