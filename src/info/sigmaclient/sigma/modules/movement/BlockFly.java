...
//    public BooleanValue searchOnly = new BooleanValue("OnlyOneSearch", false);
    ...
    public BooleanValue raytrace = new BooleanValue("Raytrace", false) {@Override public boolean isHidden() {return (type.is("Hypixel") || type.is("NCP")) && !placeTime.is("Telly");}};
    ...
    public ModeValue towerMode = new ModeValue("TowerMode", "Vanilla", new String[]{"Vanilla", "NewNCP", "NewNCP2", "NCP", "Test", "Hypixel", "Hypixel2", "Matrix", "Vulcan"}){@Override public boolean isHidden() {return !towerMove.isEnable();}};
    public ModeValue place = new ModeValue("PlaceTiming", "Legit", new String[]{"Legit", "Prev", "Post"});
    public ModeValue itemSpoofMode = new ModeValue("ItemSpoofMode", "LiteSpoof", new String[]{"LiteSpoof"});
    ...
    public NumberValue sneakTicks = new NumberValue("SneakTicks", 1, 1, 30, NumberValue.NUMBER_TYPE.INT){@Override public boolean isHidden() {
        return !sneak.isEnable();
    }};
    public NumberValue sneakdelay = new NumberValue("SneakDelay", 1, 1, 30, NumberValue.NUMBER_TYPE.INT){@Override public boolean isHidden() {
        return !sneak.isEnable();
    }};
    int stage = 0;
    ...
    //     registerValue(place);
         ...
         registerValue(sneak);
         registerValue(sneakTicks);
         registerValue(sneakdelay);
    }
    ...
    public void click() {
        ...
        boolean r = raytrace.isEnable() && (!(type.is("Hypixel") || type.is("NCP")) || placeTime.is("Telly"));
        if (r) {
    ...
    public void renderBlockCounter(){
        ...
        this.asff.animTo(this.enabled ? Sigma5AnimationUtil.AnimState.ANIMING : Sigma5AnimationUtil.AnimState.SLEEPING);
    ...
    public void bmc(int n, int n2, final float n3) {
        ...
        Sigma5DrawText.drawString(JelloFontUtil.jelloFont18, (float)(n + 10 / 2), (float)(n2 + 5), count + "", applyColor(ColorUtils.WHITE.ColorCode, n3));
        Sigma5DrawText.drawString(JelloFontUtil.jelloFont14, (float)(n + 10 / 2+ n5), (float)(n2 + 7), "Blocks", applyColor(ColorUtils.WHITE.ColorCode, 0.6f * n3));
        n += 11 / 2f + n6 / 2;
        n2 += n7;
        GL11.glPushMatrix();
...
//                }
        ...
        if(sneak.isEnable()) {
            sneaktick ++;
            if(sneaktick <= sneakTicks.getValue().intValue() ){
                mc.gameSettings.keyBindSneak.pressed = true;
            }else{mc.gameSettings.keyBindSneak.pressed = false;
            }
            if(sneaktick > sneakTicks.getValue().intValue() + sneakdelay.getValue().intValue()){
                sneaktick = 0;
            }
        }
        if(jump.isEnable() && keepY.isEnable()){
            ...
            }else if(MovementUtils.isMoving() && mc.player.onGround && mc.player.getJumpTicks() == 0 && !mc.gameSettings.keyBindJump.pressed){
                mc.player.jump();
            }
        }
        if(placeTime.is("Telly")) {
                ...
                switch (towerMode.getValue()) {
                    ...
                    case "Matrix":
                        ...
                        if (MovementUtils.isMoving()){
                            if (mc.player.getMotion().y < 0.15) {
                                mc.player.jump();
                                MovementUtils.strafing(0.2);
                                mc.getConnection().sendPacketNOEvent(new CPlayerPacket(true));
                            }
                        }else {
                            if (mc.player.onGround == false) {
                                if (mc.player.getMotion().y < 0.08 && mc.player.getMotion().y > -0.5) {
                                    mc.player.getMotion().y = -0.85;
                                }
                            }
                        }
                        break;
...
//                        event.y = event.y - (event.y % 0.015625);
                    ...
                    case "Hypixel2":
                        ...
                        if(hyp2Hold < 7){
                            ...
                            MovementUtils.setSpeed(0.241);
                            if(hyp2Tick == 1){
                                mc.player.getMotion().y = 0.33;
                            }
                            if(hyp2Tick == 2){
                                mc.player.getMotion().y = 1 - mc.player.getPosY() % 1;
                            }
                            if(hyp2Tick == 2){
                                hyp2Tick = 0;
                            }
