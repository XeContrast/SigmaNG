...
import info.sigmaclient.sigma.event.impl.other.LivingEntitiesUpdateEvent;
...
public class ESP extends Module {
   ...
   public static ShadowESP shadowESP = new ShadowESP();
    public static ModeValue mode = new ModeValue("Type", "Glow", new String[]{
            "Box Outline", "Shadow", "Glow"
    });
    ...
    @EventTarget
    public void onMotion(LivingEntitiesUpdateEvent event){
        if(mode.is("Glow")){
            if(event.getEntity().equals(mc.player)) return;
            if (isTargetEnable(event.getEntity()) && M3DUtil.isInView(event.getEntity())) {
                event.getEntity().setFlag(6, true);
            }
