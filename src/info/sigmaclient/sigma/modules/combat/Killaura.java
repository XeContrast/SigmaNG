...
public class Killaura extends Module {
    ...
    public static ModeValue autoblockMode = new ModeValue("AutoBlock", "Vanilla",
            new String[]{
                    "None", "Vanilla", "Hypixel", "NCP", "Blink", "Fake"
            });
    ...
    public void onUpdateEvent(UpdateEvent event) {
        ...
        if (!targets.isEmpty()) {
            ...
            if (!blocking && mc.player.getHeldItem(Hand.MAIN_HAND).getItem() instanceof SwordItem) {
                block();
                blockingPacket();
                blocking = true;
            }
    ...
    private void getTargets() {
        for (Entity entity : mc.world.getLoadedEntityList()) {
            if (!(entity instanceof LivingEntity e) || mc.player.getDistanceNearest(entity) > range.getValue().floatValue())
                continue;
...
//                        }
    ...
    public void onPacketEvent(PacketEvent event) {
        ...
        if (!autoblockMode.is("None") || !autoblockMode.is("Fake") || !autoblockMode.is("Blink")) {
            if (packet instanceof CPlayerDiggingPacket && blocking && !targets.isEmpty()) {
                event.setCancelled();
            }
        }
        if (autoblockMode.is("Blink")) {
            if (packet instanceof CPlayerDiggingPacket && !targets.isEmpty()) {
...
//        GlStateManager.enableLighting();
    ...
    public void unBlock() {
        switch (autoblockMode.getValue()) {
            ...
            case "Blink" -> {
                if (attackTarget == null || targets.isEmpty()) {
                    BlinkProcess.stopBlink();
